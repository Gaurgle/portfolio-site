...
- Homepage clouds are real WebGL2 volumes with internal shadowing, scroll-driven approach, rotation and evolving density. Cloud sizes and entry paths vary, with long gaps between passes and a soft blur near the camera. A large opening cloud sits behind the logo at rest with a light foreground cover, then rushes the camera during the ROOS exit until it engulfs the screen, and disperses to expose the page; ambient weather stays out of the hero. Two large ambient volumes overlap, with separate travel speeds, and a closing cloud forms over the last screens and settles along the top edge of the contact panel. Clouds condense and disperse through their density field instead of fading their opacity. Entry coverage reuses the existing nearest volume with a soft spatial mask that eases in and out by depth; there is no second ray-marching pass or content-anchored curtain. Clouds grow as they approach and pass out of view; mouse movement and content do not affect them. On desktop, slow independent drift, rotation on three axes and density evolution continue while reading. Mobile renders one smaller volume on scroll; reduced motion keeps it still. About story cards are flat, bordered panels so prose stays legible over clouds; the desktop sidebar blur fades out past the column instead of ending in a line. No cloud assets or additional packages are required. Unsupported graphics leaves the starfield visible.
